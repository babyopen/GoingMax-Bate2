...
const BusinessSlidingWindow = {
  ...
  detectHotZoneCombo: function(zodiacSeq, recentN) {
    ...
    for (var i = beginIdx; i < total; i++) {
      var sx = zodiacSeq[i].shengxiao;
      if (!sx || self.SHENGXIAO_ALL.indexOf(sx) === -1) continue;
    ...
    // 统计每个组合出现次数
    var comboCount = {};
    for (var k = 0; k < result.comboHistory.length; k++) {
      var c = result.comboHistory[k].combo;
  ...
  SW_MODIFIER_RULES: [
    ...
    // V1.5.1 新增（V1.5.3 改回 4 窗口）：次热窗口组合命中加分（次热一级 +40）
    // 逻辑：当前生肖的 4 窗口组合匹配"次热组合"（次数第二多，排除最热）时 +40
    // 阈值：secondMaxCount >= 2 触发；与最热规则互斥（currentCombo 只可能命中其一）
    { delta: 40, signal: '次热窗口组合命中',
      reasonFn: function(ctx) {
        return '窗口组合：当前(' + ctx.currentCombo + ')命中近期次热组合(max=' + ctx.secondMaxCount + ')+40';
      },
      match: function(ctx) {
        return ctx.hotComboMaxCount >= 2
          && ctx.secondMaxCount >= 2
  ...
  predict: function(historyData, options) {
    ...
    return {
      ...
      // V1.5 新增：近期热门窗口组合分析（供视图层展示"热门组合命中"信息）
      zoneComboAnalysis: hotComboResult
    };
  },
